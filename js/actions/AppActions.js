...
import { SET_AUTH, CHANGE_FORM, SENDING_REQUEST, SET_STAT, SET_STATUS } from '../constants/AppConstants';
 ...
 */
export function login(username, password, keystrokes) {
  return (dispatch) => {
    ...
    bcrypt.hash(password, salt, (err, hash) => {
      ...
      auth.login(username, hash, keystrokes, (success,stat,status,err) => {
        // When the request is finished, hide the loading indicator
        dispatch(sendingRequest(false));
        dispatch(setAuthState(success));
        dispatch(setStatState(stat));
 ...
 */
export function register(username, password, keystrokes) {
  return (dispatch) => {
    // Show the loading indicator, hide the last error
    dispatch(sendingRequest(true));
    removeLastFormError();
...
/**
 * Sets the keystroke stats state of the application - used for charting
 * @param JSON
 */
export function setStatState(newState) {
  return { type: SET_STAT, newState };
}

/**
 * Sets the kerasMessage state of the application
 * @param {string} newState VALID: User keystrokes valid. INVALID: User Keystrokes invalid
 */
export function setStatusState(newState) {
  return { type: SET_STATUS, newState };
}
 ...
 */
function requestFailed(err,dispatch) {
